...
const buildHead = (doc: jsPDF, loan: Loan) => {
  ...
  setText(doc);
  doc.text(`Nombre: ${loan.names}`, 10, 37);
...
const buildTable = (doc: jsPDF, loanDetail: LoanDetail[]) => {
  ...
  loanDetail.map((detail, index) => {
    ...
    doc.rect(23, stY, 40, 6);
    doc.text(detail.payment_date, 25, stY + 5);
    doc.rect(63, stY, 30, 6);
...
export const buildLoanPDFDoc = (loan: Loan, loanDetail: LoanDetail[]) => {
  const doc = new jsPDF("p", "mm", "A4");

  buildHead(doc, loan);
  buildTable(doc, loanDetail);
  buildContract(doc, loan);
  buildFooter(doc, loan);
